import React, { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { useUser } from '../contexts/UserContext.jsx';
import MobileContainer from '../components/MobileContainer.jsx';

export default function GoogleAuthHandler() {
  const navigate = useNavigate();
  const { login } = useUser();
  const [status, setStatus] = useState('Connecting to Google...');

  useEffect(() => {
    // Simulate OAuth Redirect/Window
    const timer = setTimeout(() => {
      setStatus('Authorizing Account...');
      const successTimer = setTimeout(() => {
        login();
        navigate('/scan');
      }, 1500);
      return () => clearTimeout(successTimer);
    }, 1000);

    return () => clearTimeout(timer);
  }, [login, navigate]);

  return (
    <MobileContainer>
      <div className="flex-1 flex flex-col items-center justify-center px-6">
        <div className="relative size-24 mb-8">
          <div className="absolute inset-0 bg-primary/20 blur-xl rounded-full animate-pulse"></div>
          <div className="relative w-full h-full glass-panel rounded-3xl flex items-center justify-center border border-white/10">
            <svg className="w-10 h-10" fill="currentColor" viewBox="0 0 24 24">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"></path>
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"></path>
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"></path>
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"></path>
            </svg>
          </div>
        </div>
        
        <h2 className="text-xl font-bold mb-2">{status}</h2>
        <div className="w-48 h-1 bg-white/10 rounded-full overflow-hidden">
          <div className="h-full bg-primary animate-[scanLine_1.5s_ease-in-out_infinite]"></div>
        </div>
        
        <button 
          onClick={() => navigate('/join')}
          className="mt-12 text-sm font-semibold opacity-50 hover:opacity-100 transition-opacity"
        >
          Cancel Login
        </button>
      </div>
    </MobileContainer>
  );
}
